...
export class OrdersService {
  ...
  async markPaid(orderId: string) {
    ...
    // A paid order also leaves the kitchen pipeline; clear kdsStage so it can't
    // re-surface on the KDS after a refetch (the live removed-event below alone
    // is not enough — the ticket query keys off kdsStage).
    const wasOnKds = order.kdsStage !== 'NONE';
  ...
  async remove(id: string) {
    ...
    // Capture KDS state BEFORE the write — a cancelled order leaves the kitchen
    // pipeline, so we also clear kdsStage to keep the DB consistent (otherwise a
    // CANCELLED order keeps a live TO_COOK/PREPARING stage and re-surfaces on the
    // KDS the next time it refetches).
    const wasOnKds = order.kdsStage !== 'NONE';
